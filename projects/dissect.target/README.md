...
```bash
just test dissect.target
```

To build source and wheel distributions:

```bash
uv build --package dissect.target --out-dir dist/dissect.target
```

The build artifacts can be found in the `dist/dissect.target/` directory.
